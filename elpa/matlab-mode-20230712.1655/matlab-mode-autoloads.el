...
With prefix ARG, enable Global Matlab-Maint minor mode if ARG is positive; otherwise, disable it.

If called from Lisp, toggle the mode if ARG is `toggle'.
Enable the mode if ARG is nil, omitted, or is a positive number.
Disable the mode if ARG is a negative number.

Matlab-Maint minor mode is enabled in all buffers where `(lambda nil Should we turn on in this
buffer? Only if in the project. (let ((dir (expand-file-name default-directory)) (ml
(file-name-directory (expand-file-name (locate-library matlab))))) (when (string= ml (substring dir
0 (min (length dir) (length ml)))) (matlab-maint-minor-mode 1))))' would do it.
...
Matlab-Shell-Gud minor mode is enabled in all buffers where `(lambda nil Should we turn on in this
buffer? Only if in a MATLAB mode. (when (eq major-mode 'matlab-mode) (matlab-shell-gud-minor-mode
1)))' would do it.
...
Matlab-Shell-Inactive-Gud minor mode is enabled in all buffers where `(lambda nil Should we turn on
in this buffer? Only if in a MATLAB mode. (when (eq major-mode 'matlab-mode)
